...
namespace Stratus.Unity.Data
{
	...
	public sealed class ScriptableSingletonAttribute : Attribute
	{
		...
		public ScriptableSingletonAttribute(string relativePath, string name)
		{
	...
	/// </summary>
	public abstract class ScriptableSingleton : StratusScriptable
	{
		/// <summary>
		...
		where T : ScriptableSingleton
	{
		...
		{
			Type type = typeof(T);
			ScriptableSingletonAttribute settings = type.GetAttribute<ScriptableSingletonAttribute>();
			if (settings == null)
			{
				throw new ScriptableSingletonAttribute.MissingException(type.Name);
			}

			string path = settings.GetProperty<string>(nameof(ScriptableSingletonAttribute.path));
			string name = settings.GetProperty<string>(nameof(ScriptableSingletonAttribute.name));
			bool hidden = settings.GetProperty<bool>(nameof(ScriptableSingletonAttribute.hidden));

			string folderPath = FileUtility.GetFolderPath(path);
			if (folderPath == null)
			{
			...
			_instance = AssetUtility.LoadOrCreateScriptableObject<T>(fullPath);
