...
namespace Stratus.Unity.Reflection
{
	...
	public class ComponentInformation : ISerializationCallbackReceiver
	{
		...
		/// </summary>
		public List<ComponentMemberInfo> memberReferences;
		...
		public Dictionary<string, ComponentMemberInfo> membersByName
		{
			get
			{
				if (_membersByName == null)
				{
					_membersByName = new Dictionary<string, ComponentMemberInfo>();
					_membersByName.AddRangeUnique((member) => member.name, this.memberReferences);

				}
				return _membersByName;
			}
		}
		private Dictionary<string, ComponentMemberInfo> _membersByName;
		...
		public bool UpdateValue(IComponentMemberInfo member)
		{
		...
		private void CreateAllMemberReferences()
		{
			...
			{
				ComponentMemberInfo memberReference = new ComponentMemberInfo(this.fields[f], this, f);
				memberReferences.Add(memberReference);
			}

			for (int p = 0; p < this.properties.Length; ++p)
			{
				ComponentMemberInfo memberReference = new ComponentMemberInfo(this.properties[p], this, p);
				memberReferences.Add(memberReference);
			}
		...
		{
			// Validate
			Predicate<ComponentMemberInfo> validate = (member) =>
			{
				return AssertMemberIndex(member);
			};

			// Iteration
			Action<ComponentMemberInfo> iterate = (member) =>
			{
			};
		...
		public bool Refresh()
		{
			...
			for (int i = 0; i < this.fields.Length; ++i)
			{
				...
				if (!this.membersByName.ContainsKey(field.Name))
				{
					...
					ComponentMemberInfo memberReference = new ComponentMemberInfo(field, this, i);
			...
			for (int i = 0; i < this.properties.Length; ++i)
			{
				...
				if (!this.membersByName.ContainsKey(property.Name))
				{
					...
					ComponentMemberInfo memberReference = new ComponentMemberInfo(property, this, i);
					this.memberReferences.Add(memberReference);
		...
		/// <returns></returns>
		private bool AssertMemberIndex(ComponentMemberInfo memberReference)
		{
		...
		/// <param name="memberReference"></param>
		/// <returns></returns>
		private bool UpdateMemberIndex(ComponentMemberInfo memberReference)
		{
